...
output_dir = "/Users/llywelyngriffith/Documents/AZ_postdoc/files_for_charlotte/github/motif_and_clip_analysis/DMAi_vs_non_DMAi/bed/SRA/150_upstream"
...
calculate_exons_length <- function(gene, strand, chr, region_start, region_end, exons_gr) {
  ...
  # 2) Find exons that overlap *in any way*
  overlapping_exons <- subsetByOverlaps(
    exons_gr, 
    region_gr, 
    ignore.strand = FALSE
  )
  ...
  # 3) Clip each overlapping exon to exactly the portion inside region_gr
  clipped_exons <- pintersect(overlapping_exons, region_gr, ignore.strand = FALSE)
  
  # 4) Merge overlapping exon segments
  merged_exons <- reduce(clipped_exons, ignore.strand = FALSE)
  
  # 5) Sum the widths
  exons_length <- sum(width(merged_exons))
...
prox_to_dist_region_list <- lapply(all_sites, function(df) {
  ...
  df2 <- df2 %>%
    select(chr, UTR_region_start, UTR_region_end, gene_name, UTR_length, strand) 
  
  return(df2)
})


output_dir = "/Users/llywelyngriffith/Documents/AZ_postdoc/files_for_charlotte/github/motif_and_clip_analysis/DMAi_vs_non_DMAi/bed/SRA/UTR_regions"
...
prox_to_exon_start_list <- lapply(proximal_sites, function(df) {
  ...
  df_joined <- df %>%
    dplyr::left_join(first_exons %>% select(gene_name, strand, first_exon_coord),
  ...
  df_final <- df_joined %>%
    select(chr, start, end, gene_name, exons_length, strand)  # Reorder columns
