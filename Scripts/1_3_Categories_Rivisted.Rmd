...
df_prmt_20_15_24 <- df_all%>% filter((
  ...
  ( ((mean_diff48>cutoff_2 | mean_diff48< -cutoff_2) & sign(mean_diff48)==sign(mean_diff24) )|              
  ((mean_diff72>cutoff_2 | mean_diff72< -cutoff_2)& sign(mean_diff72)==sign(mean_diff24) ) | 
  ((mean_diff96>cutoff_2 | mean_diff96< -cutoff_2) & sign(mean_diff96)==sign(mean_diff24) )|
  ((mean_diff96hrs_prev>cutoff_2 | mean_diff96hrs_prev< -cutoff_2) & sign(mean_diff96hrs_prev)==sign(mean_diff24) ) )))

df_prmt_20_15_48 <- df_all %>% filter((
  ((mean_diff48>cutoff_1 | mean_diff48< -cutoff_1)
  & twostep_transcript_padj_48<0.05) &
  (( (mean_diff24>cutoff_2 | mean_diff24< -cutoff_2) & sign(mean_diff48)==sign(mean_diff24) )|              
  ((mean_diff72>cutoff_2 | mean_diff72< -cutoff_2) & sign(mean_diff48)==sign(mean_diff72) )| 
  ((mean_diff96>cutoff_2 | mean_diff96< -cutoff_2) & sign(mean_diff48)==sign(mean_diff96) )|
  ((mean_diff96hrs_prev>cutoff_2 | mean_diff96hrs_prev< -cutoff_2)& sign(mean_diff48)==sign(mean_diff96hrs_prev) )) ))


df_prmt_20_15_72 <- df_all%>% filter((
  ((mean_diff72>cutoff_1 | mean_diff72< -cutoff_1)
  & twostep_transcript_padj_72<0.05) &
  ( ((mean_diff48>cutoff_2 | mean_diff48< -cutoff_2)& sign(mean_diff48)==sign(mean_diff72) ) |              
  ((mean_diff24>cutoff_2 | mean_diff24< -cutoff_2)& sign(mean_diff24)==sign(mean_diff72) ) | 
  ((mean_diff96>cutoff_2 | mean_diff96< -cutoff_2)& sign(mean_diff96)==sign(mean_diff72) ) |
  ((mean_diff96hrs_prev>cutoff_2 | mean_diff96hrs_prev< -cutoff_2)& sign(mean_diff96hrs_prev)==sign(mean_diff72) )) ))
...
df_prmt_20_15_96 <- df_all%>% filter((
  ...
  (( (mean_diff48>cutoff_2 | mean_diff48< -cutoff_2)& sign(mean_diff48)==sign(mean_diff96) ) |              
  ((mean_diff72>cutoff_2 | mean_diff72< -cutoff_2) & sign(mean_diff72)==sign(mean_diff96) )| 
  ((mean_diff24>cutoff_2 | mean_diff24< -cutoff_2) & sign(mean_diff24)==sign(mean_diff96) )|
  ((mean_diff96hrs_prev>cutoff_2 | mean_diff96hrs_prev< -cutoff_2)& sign(mean_diff96hrs_prev)==sign(mean_diff96) )) ))
...
df_prmt_20_15_96_prev <- df_all%>% filter((
  ...
  (( (mean_diff48>cutoff_2 | mean_diff48< -cutoff_2)& sign(mean_diff48)==sign(mean_diff96hrs_prev) ) |              
  ((mean_diff72>cutoff_2 | mean_diff72< -cutoff_2) & sign(mean_diff72)==sign(mean_diff96hrs_prev) )| 
  ((mean_diff96>cutoff_2 | mean_diff96< -cutoff_2) & sign(mean_diff96)==sign(mean_diff96hrs_prev) )|
  ((mean_diff24>cutoff_2 | mean_diff24< -cutoff_2) & sign(mean_diff24)==sign(mean_diff96hrs_prev) )) ))
...
df_prmt_20_15_70.1 <- df_prmt_20_15_70 %>% group_by(gene_ID) %>% slice_max(abs(mean_diff_across),with_ties=FALSE)
```
...
for (gene in df_prmt_20_15_70.1$gene_ID) {
  if (df_prmt_20_15_70.1[df_prmt_20_15_70.1$gene_ID==gene,]$mean_diff_across>0) {
      feature <- all_other_features[all_other_features$gene_ID==gene,] %>% filter(mean_diff_across<0) %>% slice_min(mean_diff_across,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)} else {
      feature <- all_other_features[all_other_features$gene_ID==gene,] %>% filter(mean_diff_across>0) %>%  slice_max(mean_diff_across,n=1,with_ties=FALSE)
...
table(tally(df_prmt_20_15_70_PO)$n)
...
for (gene in df_siRNA_20.CF$gene_ID) {
  if (df_siRNA_20.CF[df_siRNA_20.CF$gene_ID==gene,]$mean_diffsiCFIM25>0) {
      feature <- df_siRNA_20.CF_other[df_siRNA_20.CF_other$gene_ID==gene,] %>% filter(mean_diffsiCFIM25<0) %>%  slice_min(mean_diffsiCFIM25,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)} else {
      feature <- df_siRNA_20.CF_other[df_siRNA_20.CF_other$gene_ID==gene,] %>%filter(mean_diffsiCFIM25>0) %>%  slice_max(mean_diffsiCFIM25,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)}}
...
for (gene in df_siRNA_20.EL$gene_ID) {
  if (df_siRNA_20.EL[df_siRNA_20.EL$gene_ID==gene,]$mean_diffsiELAVL1>0) {
      feature <- df_siRNA_20.EL_other[df_siRNA_20.EL_other$gene_ID==gene,] %>% filter(mean_diffsiELAVL1<0) %>%  slice_min(mean_diffsiELAVL1,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)} else {
      feature <- df_siRNA_20.EL_other[df_siRNA_20.EL_other$gene_ID==gene,] %>%filter(mean_diffsiELAVL1>0) %>%  slice_max(mean_diffsiELAVL1,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)}}
...
for (gene in df_siRNA_20.SM$gene_ID) {
  if (df_siRNA_20.SM[df_siRNA_20.SM$gene_ID==gene,]$mean_diffsiSam68>0) {
      feature <- df_siRNA_20.SM_other[df_siRNA_20.SM_other$gene_ID==gene,] %>% filter(mean_diffsiSam68<0) %>%  slice_min(mean_diffsiSam68,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)} else {
      feature <- df_siRNA_20.SM_other[df_siRNA_20.SM_other$gene_ID==gene,] %>% filter(mean_diffsiSam68>0) %>%  slice_max(mean_diffsiSam68,n=1,with_ties=FALSE)
     features_to_add<- append(features_to_add,feature$unique_featureID)}}
...
## removed all genes found in Prev 2 categories
...
write_csv(df_control_pairs,'../Outputs/All_categories_df.csv')
```
...
Take abs value so sign doesnt influence look at max value between the 3 (if Na in one for that value it returns -inf so for sure wont be picked as the one with biggest value)
```{r}
ds$RowMEAN <- 0
...
add row mean for prmt reg (the one for reg is just value of max effect reg)
```{r}
...
for (gene in ds$gene_ID) {
  ...
  ifelse ((ds[ds$gene_ID==gene,]$strand=='+') ,
    {if(ds[ds$gene_ID==gene,]$start[1] >ds[ds$gene_ID==gene,]$start[2] ) {
...
ds <- ds %>% mutate(gene_ID= unlist(strsplit(gene_ID,'_'))[1])
#write_csv(ds,'../Outputs/All_categories_DP_LS_RM.csv')

ds <- ds %>% select(-RowMEAN)

table(ds$category,ds$Effect)
```

```{r}
write_csv(ds,'../Outputs/All_categories_DP_LS.csv')
