...
write_csv(df_all,'../Data/Outputs/Unfiltered_Formatted_All_Data_Sets_Final.csv')
...
keep only paired genes, take pair of features that have max and min change based on mean of all observation in PRMT ds
...
This is just old way of categ, to check I had equal results to prev script
...
might remove 70 check if low number
```{r}
...
Again keep pairs only, I did this with a fx for each siRNA separately as such the specific mean diff for ds is used.
Still keeping the 2 with opposite sign and biggest value
...
merged features for diff siRNA ds, then removed genes present in PRMT category.
IMP: there is one gene (SLC25A46_ENSG00000164209.17) that has 3 features. This is because I checked for most 'important' ones separately and 2 siRNA ds resulted in 1feat shared and 1 unique per siRNA selected.
...
#Tried with mean both, but everything passed so useless 
...
#write_csv(df_control_final,'../Data/Outputs/Control_sites')
```
...
```{r}
BACKGROUND <- unlist(strsplit(unique(df_all$gene_ID),split = '_'))[seq(1,length(unique(df_all$gene_ID))*2,2)]
Control_sites_genes <-unlist(strsplit(unique(df_control_final$gene_ID),split = '_'))[seq(1,length(unique(df_control_final$gene_ID))*2,2)]
Regulatable_sites_genes <- unlist(strsplit(unique(Regulatable_sites_noPRMT$gene_ID),split = '_'))[seq(1,length(unique(Regulatable_sites_noPRMT$gene_ID))*2,2)]
PRMT_regulated_genes <-unlist(strsplit(unique(df_prmt_20_15_70_PO$gene_ID),split = '_'))[seq(1,length(unique(df_prmt_20_15_70_PO$gene_ID))*2,2)]
```

```{r}
df = data.frame(matrix(nrow = 5177, ncol =4))
colnames(df) <- c('PRMT_regulated_genes','Regulatable_sites_genes','Control_sites_genes','BACKGROUND')


df$Control_sites_genes <- append(Control_sites_genes, rep(NA,5177-length(Control_sites_genes)))
df$'_BACKGROUND' <- BACKGROUND
df$Regulatable_sites_genes <- append(Regulatable_sites_genes, rep(NA,5177-length(Regulatable_sites_genes)))
df$PRMT_regulated_genes <- append(PRMT_regulated_genes, rep(NA,5177-length(PRMT_regulated_genes)))


write_xlsx(df,'../Data/Outputs/gene_list.xlsx')


table(is.na(df$Control_sites_genes))
```
...
```{r}
PRMT_gl = data.frame(matrix(nrow = 5177, ncol =2))
colnames(PRMT_gl) <- c('PRMT_regulated_genes','BACKGROUND')


PRMT_gl$'_BACKGROUND' <- BACKGROUND
PRMT_gl$PRMT_regulated_genes <- append(PRMT_regulated_genes, rep(NA,5177-length(PRMT_regulated_genes)))


write_xlsx(PRMT_gl,'../Data/Outputs/PRMT_gl.xlsx')


table(is.na(PRMT_gl$PRMT_regulated_genes))
```

```{r}
regulatable_gl = data.frame(matrix(nrow = 5177, ncol =2))
colnames(regulatable_gl) <- c('Regulatable_sites_genes','BACKGROUND')


regulatable_gl$'_BACKGROUND' <- BACKGROUND
regulatable_gl$Regulatable_sites_genes <- append(Regulatable_sites_genes, rep(NA,5177-length(Regulatable_sites_genes)))


write_xlsx(regulatable_gl,'../Data/Outputs/regulatable_gl.xlsx')


table(is.na(regulatable_gl$Regulatable_sites_genes))
```


```{r}

ctrl_gl = data.frame(matrix(nrow = 5177, ncol =2))
colnames(ctrl_gl) <- c('Control_sites_genes','BACKGROUND')


ctrl_gl$'_BACKGROUND' <- BACKGROUND
ctrl_gl$Control_sites_genes <- append(Control_sites_genes, rep(NA,5177-length(Control_sites_genes)))


write_xlsx(ctrl_gl,'../Data/Outputs/Control_sites_genes.xlsx')


table(is.na(ctrl_gl$Control_sites_genes))
...
write_csv(df_prmt_20_15_70_PO,'../Data/Outputs/PRMT_reg.csv')
```
