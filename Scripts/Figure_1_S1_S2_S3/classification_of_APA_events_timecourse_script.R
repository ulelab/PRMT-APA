...
# Identify iAPA sites within 'TUTR' sites and filter them out next
TUTR_sites_list <- lapply(TUTR_sites_list, function(df) {
  df %>%
    group_by(ensembl_ID) %>%
    mutate(
...
#iAPA rather than TUTR sites - should have been filtered out from drimseq but contrasting annotation carried over
...
combined_tallies$UTR_type <- factor(combined_tallies$UTR_type, levels = c('TUTR', 'ALE', 'MIXED'))
...
# Convert 'UTR_type' to factor with specified levels for consistent plotting
combined_tallies_ctrl$UTR_type <- factor(combined_tallies_ctrl$UTR_type, levels = c('TUTR', 'ALE', 'MIXED'))
...
combined_prox_dist_shift <- bind_rows(combined_prox_dist_shift, all_utr_df) %>% filter(UTR_type != 'iAPA')

# **Step 8:** Ensure 'UTR_type' and 'pA_type' are factors with specified levels
combined_prox_dist_shift$UTR_type <- factor(combined_prox_dist_shift$UTR_type, levels = c('all','TUTR', 'ALE', 'MIXED'))
