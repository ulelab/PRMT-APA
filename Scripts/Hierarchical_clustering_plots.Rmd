...
## let's try with abslute values

```{r}

ds.2 <- ds[complete.cases(ds[,7:14]),]
ds.2.2 <- abs(ds.2[,7:14])

ds.2 <- mutate(ds.2, color = ifelse(ds.2$category=='Control_sites','red' ,  ifelse(ds.2$category=='Regulatable_sites','blue' , "yellow")))

ds.2 <- as.matrix(ds.2[,c(7:14,1,87,88)])
ds.2.2 <- as.matrix(ds.2.2)


rownames(ds.2) <-ds.2[,'unique_featureID']
rownames(ds.2.2) <-ds.2[,'unique_featureID']
...
Then do clustering with hclust method complete should be best
```{r}
distSamples <- dist(ds.2.2)
hc <- hclust(distSamples,method = 'average')
hc
```
