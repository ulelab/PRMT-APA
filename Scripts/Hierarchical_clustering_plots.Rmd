...
### try to get out labels cate and plot
...
ds.l <-ds_L %>%  mutate(color_categ = ifelse(category=='Control_sites','red' ,  ifelse(category=='Regulatable_sites','blue' , "yellow")))

ds.l <- as.matrix(ds.l[,c(7:14)])
...
```
```{r}
ds_S <- ds %>% filter(Effect!='shortening')
ds_S <- ds_S[complete.cases(ds_S[,7:14]),]

ds.s <-ds_S %>%  mutate(color_categ = ifelse(category=='Control_sites','red' ,  ifelse(category=='Regulatable_sites','blue' , "yellow")))

ds.s <- as.matrix(ds.s[,c(7:14)])

f1 = colorRamp2(seq(-1,1, length = 3), c("red", "white", "dark blue"))

#rowAnnotation(foo2 = 18:1, bar2 = anno_barplot(runif(18)) 
Heatmap(ds.s, name = "Lengthening heatmap",col=f1,row_km  = 3,
