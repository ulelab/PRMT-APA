...
ggplot(all_sites_table, aes(x = category, y = prox_usage, fill = category)) +
  ...
  scale_fill_manual(values = c(
    ...
    "DMAi" = "#ff594e",  # orange
    "non-DMAi" = "#1A85FF"  # green
  ))

# Ensure order so effect-size direction is consistent
all_sites_table <- all_sites_table %>%
  mutate(category = factor(category, levels = c("control","DMAi","non-DMAi")))

# Pairwise Wilcoxon vs control (two-sided), BH-adjusted across the two tests
pw_tbl <- all_sites_table %>%
  pairwise_wilcox_test(prox_usage ~ category,
                       ref.group = "control",
                       p.adjust.method = "BH") %>%
