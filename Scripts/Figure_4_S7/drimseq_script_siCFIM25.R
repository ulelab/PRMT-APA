...
# Example:
setwd("/ulelab/PRMT-APA/Scripts/Figure_4_S7")
...
pas_atlas_file <- '../../Data/Figure_4_S7/pA_atlas/merged_polya.filteredunique.annotated.bed'
...
counts_dir <- "../../Data/Figure_4_S7/counts"  # Directory containing count files
...
metadata_file <- "../../Data/Figure_4_S7/drimseq_metadata.txt"
...
conditions_list <- list(
  'siCFIM25_DMSO' = c('ctrl_DMSO', 'CFIM25_DMSO'),
  'siCFIM25_DMAi' = c('ctrl_DMSO', 'CFIM25_SDMAi_ADMAi')
...
dir.create("../../Data/Figure_4_S7/APA_CSV_files/", showWarnings = FALSE)
dir.create("../../Data/Figure_4_S7/APA_CSV_files/positional_info", showWarnings = FALSE)

for (i in seq_along(condition_results)) {
  cond_name <- condition_results[[i]]$condition_name
  
  # Define output file paths
  all_sites_file <- paste0("../../Data/Figure_4_S7/APA_CSV_files/all_polyA_sites_", cond_name, "_with_change_in_usage.csv")
  sig_sites_file <- paste0("../../Data/Figure_4_S7/APA_CSV_files/sig_polyA_sites_", cond_name, "_with_change_in_usage.csv")
...
for (i in seq_along(condition_results)) {
  cond_name <- condition_results[[i]]$condition_name
  output_file <- paste0("../../Data/Figure_4_S7/APA_CSV_files/positional_info/top2_", cond_name, "_sites_with_positional_info.csv")
...
#isolate just siCFIM25 conditions for comparative boxplot
siCFIM25_combined_shifted_pas = combined_shifted_pas %>% filter(condition == 'siCFIM25_DMSO' | condition == 'siCFIM25_DMAi')
...
filtered_data <- siCFIM25_combined_shifted_pas %>%
  group_by(condition) %>%
...
x_axis_labels <- c('siCFIM25 + DMSO', 'siCFIM25 + DMAi')  # Custom x-axis labels

# Create the boxplot
shift_boxplot <- ggplot(filtered_data, aes(x = condition, y = prox_to_dist_shift, fill = condition)) +
  geom_boxplot() +
  scale_fill_manual(values = c('#98D1FF','#e77171'), labels = x_axis_labels) +  # Custom legend labels and colors
...
# Assess difference in prox to dist shifts in siCFIM25_DMSO and siCFIM25_DMAi
conds <- c("siCFIM25_DMSO", "siCFIM25_DMAi")
...
siCFIM25_DMSO_combined_shifted_pas_mean = mean(siCFIM25_combined_shifted_pas$prox_to_dist_shift[siCFIM25_combined_shifted_pas$condition == "siCFIM25_DMSO"],na.rm = TRUE) # -0.22
siCFIM25_DMAi_combined_shifted_pas_mean = mean(siCFIM25_combined_shifted_pas$prox_to_dist_shift[siCFIM25_combined_shifted_pas$condition == "siCFIM25_DMAi"],na.rm = TRUE) # -0.12
