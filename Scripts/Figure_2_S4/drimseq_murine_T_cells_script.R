...
pas_atlas_file <- '../../Data/Figure_2_S4/pA_atlases/mouse/merged_polya.filteredunique.annotated.bed'
...
counts_dir <- "../../Data/Figure_2_S4/counts/murine_T_cells/"  # Directory containing count files
...
metadata_file <- "../../Data/Figure_2_S4/drimseq_murine_T_cell_metadata.txt"
...
conditions_list <- list(
  'Naive_vs_Polarised' = c('Unpolarised', 'DMSO'),
  'Polarised_DMSO_vs_DMAi' = c('DMSO', 'SDMAi_ADMAi'),
  'Naive_vs_Polarised_DMAi' = c('Unpolarised', 'SDMAi_ADMAi')
...
dir.create("../../Data/Figure_2_S4/APA_CSV_files/murine_T_cells", showWarnings = FALSE)
dir.create("../../Data/Figure_2_S4/APA_CSV_files/murine_T_cells/positional_info", showWarnings = FALSE)

for (i in seq_along(condition_results)) {
  cond_name <- condition_results[[i]]$condition_name
  
  # Define output file paths
  all_sites_file <- paste0("../../Data/Figure_2_S4/APA_CSV_files/murine_T_cells/all_polyA_sites_", cond_name, "_with_change_in_usage.csv")
  sig_sites_file <- paste0("../../Data/Figure_2_S4/APA_CSV_files/murine_T_cells/sig_polyA_sites_", cond_name, "_with_change_in_usage.csv")
...
#Naive vs Polarised DMSO or DMAi comparison
DMAi_comparison_data = filtered_data %>% filter(condition == 'Naive_vs_Polarised' | condition == 'Naive_vs_Polarised_DMAi')

#define colour scheme
top2_colours = c('#a0dcff','#e77171')

# Define custom labels for the x-axis and legend
x_axis_labels <- c('Polarised + DMSO', 'Polarised + DMAi')  # Custom x-axis labels

# Create the boxplot
shift_boxplot <- ggplot(DMAi_comparison_data, aes(x = condition, y = prox_to_dist_shift, fill = condition)) +
  geom_boxplot() +
...
# Assess difference in prox to dist shifts in Naive vs Polarised and Naive vs Polarised DMAi
conds <- c("Naive_vs_Polarised", "Naive_vs_Polarised_DMAi")
DMSO_vs_DMAi <- combined_shifted_pas %>%
...
Pol_DMSO_prox_to_dist_shift_mean = mean(combined_shifted_pas$prox_to_dist_shift[combined_shifted_pas$condition == "Naive_vs_Polarised"],na.rm = TRUE) # -0.1
Pol_DMAi_prox_to_dist_shift_mean = mean(combined_shifted_pas$prox_to_dist_shift[combined_shifted_pas$condition == "Naive_vs_Polarised_DMAi"],na.rm = TRUE) # 0.02
