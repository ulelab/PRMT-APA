...
conditions <- c("GP2D", "H838","HCT116_p53minus","HCT116_p53plus","LU99",
                "MCF7","PANC0403","SKMEL2","SUM149PT","U2OS")  # Update this vector as per your conditions
...
strip_labels <- c(
  "GP2D"      = "GP2D",
  "H838"      = "NCI-H838",
  "HCT116_p53minus" = "HCT116 p53 -/-",
  "HCT116_p53plus" = "HCT116 p53 +/+",
  "LU99"      = "LU-99",
  "MCF7" = "MCF7",
  "PANC0403"      = "PANC0403",
  "SKMEL2"      = "SK-MEL-2",
...
#filter to only keep All, TUTR and ALE for this multi-cell line plot
combined_prox_dist_shift = combined_prox_dist_shift %>% filter(UTR_type == 'all' | UTR_type == 'TUTR' | UTR_type == 'ALE')

# **Step 8:** Ensure 'UTR_type' and 'pA_type' are factors with specified levels
combined_prox_dist_shift$UTR_type <- factor(combined_prox_dist_shift$UTR_type, levels = c('all','TUTR', 'ALE'))

# Get the first four colors from the Set2 palette
set2_colors <- brewer.pal(8, "Set2")[1:2]

# Construct the color vector
color_vector <- rep(c('#9C2A2A',set2_colors),times = 10)
