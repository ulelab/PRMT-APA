...
conditions_list <- list(
  'HUB_N_DMSO_vs_T_DMSO' = c('HUB_N_DMSO', 'HUB_EGFR_T_DMSO'),
  'HUB_N_DMSO_vs_N_DMAi' = c('HUB_N_DMSO', 'HUB_N_SA'),
  'HUB_T_DMSO_vs_T_DMAi' = c('HUB_EGFR_T_DMSO', 'HUB_EGFR_T_SA'),
  'HUB_N_DMSO_vs_T_DMAi' = c('HUB_N_DMSO', 'HUB_EGFR_T_SA'),
  'Tempus_EGFR_T_DMSO_vs_DMAi' = c('Tempus_EGFR_T_DMSO', 'Tempus_EGFR_T_SA'),
  'Tempus_KRAS_T_DMSO_vs_DMAi' = c('Tempus_KRAS_T_DMSO', 'Tempus_KRAS_T_SA')
  # Add more conditions as needed, e.g.,
  # '120hrs' = c('DMSO_120hr', 'ADMAi_SDMAi_120hr')
)
...
combined_shifted_pas$condition <- factor(combined_shifted_pas$condition, levels = unique(combined_shifted_pas$condition))

# Step 14: Plot the proximal to distal usage shift
# Remove top 5% highest absolute shifts in each group to reduce the impact of outliers
filtered_data <- combined_shifted_pas %>%
  group_by(condition) %>%
  mutate(abs_shift = abs(prox_to_dist_shift)) %>%
  arrange(desc(abs_shift)) %>%
  mutate(rank = row_number()) %>%
  filter(rank > 0.05 * max(rank)) %>%
  ungroup() %>%
  select(-abs_shift, -rank) %>% # Remove temporary columns
  group_by(gene_name, condition) %>% dplyr::slice(1) %>% ungroup() # Only one row per gene

#just cancer DMAi table
cancer_DMAi_data = filtered_data %>% filter(condition == 'HUB_T_DMSO_vs_T_DMAi' | condition == 'Tempus_EGFR_T_DMSO_vs_DMAi' | condition == 'Tempus_KRAS_T_DMSO_vs_DMAi')

#define colour scheme
top2_colours = c('#e77171','#e77171','#e77171')

# Define custom labels for the x-axis and legend
x_axis_labels = c('HUB EGFR', 'Tempus EGFR','Tempus KRAS')  # Custom x-axis labels

# Create the boxplot
shift_boxplot = ggplot(cancer_DMAi_data, aes(x = condition, y = prox_to_dist_shift, fill = condition)) +
  geom_boxplot() +
  scale_fill_manual(values = top2_colours, labels = x_axis_labels) +  # Custom legend labels and colors
  scale_x_discrete(labels = x_axis_labels) +  # Custom x-axis labels
  ...
  labs(fill = "Condition")  # Legend title

# Save the plot to a file
# Output: A JPEG file of the boxplot
ggsave("cancer_DMAi_proximal_to_distal_usage_shift_boxplot.jpg", plot = shift_boxplot, width = 10, height = 8, dpi = 300)

#just HUB table
HUB_data = filtered_data %>% filter(condition == 'HUB_N_DMSO_vs_N_DMAi' | condition == 'HUB_T_DMSO_vs_T_DMAi')

#define colour scheme
top2_colours = c('#f0b8b8','#e77171')

# Define custom labels for the x-axis and legend
x_axis_labels = c('HUB Normal', 'HUB EGFR Tumour')  # Custom x-axis labels
legend_labels = c('HUB Normal + DMAi', 'HUB EGFR Tumour + DMAi')  # Custom x-axis labels

# Create the boxplot
shift_boxplot = ggplot(HUB_data, aes(x = condition, y = prox_to_dist_shift, fill = condition)) +
  geom_boxplot() +
  scale_fill_manual(values = top2_colours, labels = legend_labels) +  # Custom legend labels and colors
  scale_x_discrete(labels = x_axis_labels) +  # Custom x-axis labels
  ylab('Proximal to Distal Usage Shift') +
  geom_hline(yintercept = 0, linetype = "dotted", color = "black", size = 0.8) +
  theme_classic() +
...
# Assess difference in prox to dist shifts in Hub N DMSO vs H N DMAi and Hub T DMSO vs Hub T DMAi
conds <- c("HUB_N_DMSO_vs_N_DMAi", "HUB_T_DMSO_vs_T_DMAi")
hub_two <- combined_shifted_pas %>%
  filter(condition %in% conds) %>%
  select(gene_name, condition, prox_to_dist_shift) %>%
  droplevels()

## unpaired Wilcox test
wilcox_unpaired <- wilcox_test(
  prox_to_dist_shift ~ condition,
  data = hub_two,
  alternative = "two.sided"
) %>% add_significance()

#calculate mean values
HUB_N_DMSO_vs_DMAi_prox_to_dist_mean = mean(combined_shifted_pas$prox_to_dist_shift[combined_shifted_pas$condition == "HUB_N_DMSO_vs_N_DMAi"],na.rm = TRUE) # 0.05
HUB_T_DMSO_vs_DMAi_prox_to_dist_mean = mean(combined_shifted_pas$prox_to_dist_shift[combined_shifted_pas$condition == "HUB_T_DMSO_vs_T_DMAi"],na.rm = TRUE) # 0.08

##compare baseline (DMSO) proximal pA usage between HUB Normal and HUB Tumour
proximal_usage_HUB_table = combined_shifted_pas %>% filter(condition == 'HUB_N_DMSO_vs_T_DMSO') %>% filter(pA_type == 'proximal pA')

#convert data for plot
long_data <- proximal_usage_HUB_table %>%
  pivot_longer(
    cols = c(pA_usage_control, pA_usage_treatment),  # Columns to pivot into longer format
    names_to = "organoid",         # New column for the former column names
    values_to = "prox_usage"       # New column for the values
  ) %>%
  mutate(organoid = recode(organoid, 
                           pA_usage_control = "HUB Normal", 
                           pA_usage_treatment = "HUB EGFR Tumour"))

#relevel organoid variable
long_data$organoid = factor(long_data$organoid, levels = c('HUB Normal','HUB EGFR Tumour'))

#ggplot
ggplot(long_data, aes(x = organoid, y = prox_usage, fill = organoid)) +
  geom_boxplot() +
  scale_fill_manual(values = c('#f0b8b8','#e77171')) +
  ylab('proximal pA usage') +
  xlab('Organoid') +
  theme_classic() +
  theme(       
    axis.title = element_text(size = 16),                        # Increase axis title size
    axis.text.x = element_text(angle = 45,hjust = 0.8,vjust = 0.8, size = 14),
    axis.text.y = element_text(size = 16), # Increase axis text (ticks) size
...
wilcox_res <- wilcox.test(prox_usage ~ organoid,
                          data        = long_data,
                          alternative = "two.sided")   %>% add_significance()
