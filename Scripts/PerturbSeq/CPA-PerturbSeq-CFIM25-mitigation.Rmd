...
files <- list.files(path = folder_path, pattern = "\\.bed$", full.names = TRUE,recursive=TRUE)

# Filter for *not* the upstream bed regions
files <- files[grepl("mitigation",files)]
...
pseq_gr = GRanges(seqnames = Rle(pseq$seqnames),
                ranges = IRanges(start = pseq$start, end = pseq$end),
...
left_join(original_site_count, genes_with_both_summary, by=("condition"="condition")) %>%
  ...
  fwrite(.,file="percent-found-in-cpa-perturb-hek-MITIGATION-50nt-window.csv")
```
...
```
```{r}
kd_list = Idents(hek) %>% unique() %>% length()
...
pdf("../../Outputs/cfim-mitigation-cpa-perturbseq-heatmap.pdf")
# Draw the heatmap
...
res_dist <- res %>% filter(site_type == "proximal")
...
res_dist <- res_dist %>%
  mutate(kd = factor(kd, levels = order_levels),
         condition = factor(condition, levels = c("siCFIM25full","siCFIM25part","siCFIM25no")))
...
# 6. Save to PDF
pdf("../../Outputs/cfim-mitigation-cpa-perturbseq-boxplots-ordered-proximal.pdf", width = 8, height = 5)
print(p)
dev.off()
```
...
```{r}
res_dist %>% filter(kd=="NUDT21") %>%
  ggplot(.,aes(x=condition, y=percent.2)) + geom_boxplot() + theme_few() + ylab("Distal usage in CPA-Perturbseq HEK293 control") + xlab("") + theme(aspect.ratio=1)
ggsave("../../Outputs/CPA-perturb-site-usage-cntrl.pdf")
```
