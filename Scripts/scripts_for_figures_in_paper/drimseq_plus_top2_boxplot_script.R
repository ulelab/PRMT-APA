...
conditions_list <- list(
  'DMAi_24hrs' = c('DMSO_24hr', 'ADMAi_SDMAi_24hr'),
  'DMAi_48hrs' = c('DMSO_48hr', 'ADMAi_SDMAi_48hr'),
  'DMAi_72hrs' = c('DMSO_72hr', 'ADMAi_SDMAi_72hr'),
  'DMAi_96hrs' = c('DMSO_96hr', 'ADMAi_SDMAi_96hr')
...
for (i in seq_along(condition_results)) {
  ...
  all_sites_file <- paste0("CSV_files/all_polyA_sites_", cond_name, "_with_change_in_usage.csv")
  sig_sites_file <- paste0("CSV_files/sig_polyA_sites_", cond_name, "_with_change_in_usage.csv")
...
top2_colours = c('#F1898F','#FF555F','#D21823','#9B030D')

# Define custom labels for the x-axis and legend
x_axis_labels <- c('DMAi 24hrs', 'DMAi 48hrs','DMAi 72hrs', 'DMAi 96hrs')  # Custom x-axis labels
legend_labels <- c('DMAi 24hrs', 'DMAi 48hrs','DMAi 72hrs', 'DMAi 96hrs')  # Custom legend labels
...
shift_boxplot <- ggplot(filtered_data, aes(x = condition, y = prox_to_dist_shift, fill = condition)) +
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 0.9),  # Rotate x-axis labels
