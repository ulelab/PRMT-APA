...
Ctrl_genes <- unique(df[df$category=='Regulatable_sites',]$gene_ID) # interesting genes
Regulatable_genes <- unique(df[df$category=='Control_sites',]$gene_ID) # interesting genes
...
Ctrl_genes_symbol <- unlist(strsplit(Ctrl_genes,'_'))[seq(1,length(Ctrl_genes)*2,2)]
Regulatable_genes_symbol <- unlist(strsplit(Regulatable_genes,'_'))[seq(1,length(Regulatable_genes)*2,2)]
```
...
ds$PRMT_reg <- append(PRMT_genes_symbol, rep(NA,5177-length(PRMT_genes_symbol)))
ds$Regulatable_sites <- append(Regulatable_genes_symbol, rep(NA,5177-length(Regulatable_genes_symbol)))
ds$Control_sites <- append(Ctrl_genes_symbol, rep(NA,5177-length(Ctrl_genes_symbol)))
...
geneList.1 <- factor(as.integer(gene_universe_symbol %in% Regulatable_genes_symbol ))
geneList.2 <- factor(as.integer(gene_universe_symbol %in% Ctrl_genes_symbol ))
...
set.seed(1234)
library(multtest)
allRes <- GenTable(GOdata, weight = resultFis, topNodes = 20)
#allRes$weight
...
```{r}
library(ggplot2)
ggplot(allRes[1:20,], aes(x=-log10(weight), y=reorder(paste(GO.ID,Term),-log10(weight)),color=log10(weight),size=Significant) )+scale_color_gradient(low = "magenta",high='blue')+
  xlab('') +
ylab("GO")+
...
```
```{r}
ggplot(allRes[1:20,], aes(x=adj, y=reorder(paste(GO.ID,Term),-adj),color=adj,size=Significant) )+scale_color_gradient(low = "magenta",high='blue')+
  xlab('') +
ylab("GO")+
    geom_point()+theme_bw()+labs(colour='weight')+ggtitle('GO results PRMT-reg')
...
ML_GO_MCODE <- read_csv('../GO_analysis/ML_GO_MCODE.csv')
...
ML_GO_MCODE_top20 <- ML_GO_MCODE[order(ML_GO_MCODE$Enrichment),][1:20,]

ML_GO_MCODE_HM <- ML_GO_MCODE_top20 %>% mutate(Control_sites= ifelse(Name=='Control_sites',1,0))
ML_GO_MCODE_HM <- ML_GO_MCODE_HM %>% mutate(Regulatable_sites= ifelse(Name=='Regulatable_sites',1,0))
ML_GO_MCODE_HM <- ML_GO_MCODE_HM %>% mutate(PRMT_reg= ifelse(Name=='PRMT_reg',1,0))


ML_GO_MCODE_HM <- as.matrix(ML_GO_MCODE_HM[,c(20,21,19)])

rownames(ML_GO_MCODE_HM) <- paste(ML_GO_MCODE_top20$GO,ML_GO_MCODE_top20$Description)

f1 = colorRamp2(seq(min(-ML_GO_MCODE_top20$LogP), max(-ML_GO_MCODE_top20$LogP), length = 3), c("red", "white", "dark blue"))
heatmap(ML_GO_MCODE_HM,col=f1)


#GO_results_ordered <- GO_results_ordered[1:10,]
plot_prmt <- ggplot(GO_results_ordered, aes(x=- LogP, y=reorder(paste(GO,Description),-LogP),colour=-LogP,size=`#GeneInGO`) )+scale_color_gradient2(high = "#DC143C")+
  xlab("-log(P)") +
ylab("GO")+
...
```{r}
PRMT_GO_MCODE_ord <- PRMT_GO_MCODE[order(PRMT_GO_MCODE$LogP),]
#GO_results_ordered <- GO_results_ordered[1:10,]
...
```{r}
REG_GO_MCODE_ord <- REG_GO_MCODE[order(REG_GO_MCODE$LogP),]
...
```{r}
CTRL_GO_MCODE_ord <- CTRL_GO_MCODE[order(CTRL_GO_MCODE$LogP),]
