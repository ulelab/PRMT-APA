...
library(writexl)
write_xlsx(ds,'~/Desktop/GO_list.xlsx')


PRMT_genes_symbol
gene_universe_symbol
```
...
```{r}
geneList <- factor(as.integer(gene_universe_symbol %in% PRMT_genes_symbol ),levels = c(0,1))
geneList <- as.vector(x=rep(1,length(PRMT_genes_symbol)))
names(geneList) <- PRMT_genes_symbol

geneList.1 <- as.vector(x=rep(2,length(Regulatable_genes_symbol)))
names(geneList.1) <- Regulatable_genes_symbol

geneList.2 <- as.vector(x=rep(3,length(Ctrl_genes_symbol)))
names(geneList.2) <- Ctrl_genes_symbol

geneList.3 <-gene_universe_symbol[ gene_universe_symbol %in% c(Ctrl_genes_symbol,PRMT_genes_symbol,Regulatable_genes_symbol) ]
geneList.univ <- as.vector(x=rep(0,length(geneList.3)))
names(geneList.univ) <- geneList.3

table(as.factor(c(geneList,geneList.1,geneList.2,geneList.univ)))

genes <- as.factor(c(geneList,geneList.1,geneList.2,geneList.univ))
```
...
```{r}
library(ggplot2)
ggplot(allRes[1:20,], aes(x=weight, y=reorder(paste(GO.ID,Term),-weight),color=weight,size=Significant) )+scale_color_gradient(low = "magenta",high='blue')+
  xlab('') +
ylab("GO")+
...
```{r}
set.seed(1234)
allRes.1 <- GenTable(GOdata, weight = resultClassic, topNodes = 300)
allRes$weight
allRes.1$weight <- as.numeric(allRes.1$weight)
allRes.1$adj <- p.adjust(allRes.1$weight,'BH')

```

```{r}
ggplot(allRes.1[1:20,], aes(x=adj, y=reorder(paste(GO.ID,Term),-adj),color=adj,size=Significant) )+scale_color_gradient(low = "magenta",high='blue')+
  xlab('') +
ylab("GO")+
    geom_point()+theme_bw()+labs(colour='weight')+ggtitle('GO results PRMT-reg')
